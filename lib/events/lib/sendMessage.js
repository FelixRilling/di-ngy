 ...
 * @param {Array<any>} files
 */
const performChecks = function (app, msg, text, code, files, events) {
    if (text.length === 0) {
...
const isPromise = val => isFunction(val.then);
...
module.exports = function (app, msg, data) {
    ...
    const events = defaultsDeep(data[3], eventsDefault);

    if (isPromise(data[0])) {
        text
            .then(resolvedText => {
                app.log.debug("TextAsync");
                performChecks(app, msg, resolvedText, code, files, events);
            })
            .catch(err => {
        ...
        performChecks(app, msg, text, code, files, events);
    } else {
        app.log.warning("StrangeType", text, typeof text);
        performChecks(app, msg, String(data[0]), code, files, events);
    }
};
